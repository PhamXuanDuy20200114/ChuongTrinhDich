...
Type *makeStringType(int stringSize, Type *elementType)
{
  Type *type = (Type *)malloc(sizeof(Type));
  type->typeClass = TP_STRING;
  type->arraySize = stringSize;
  type->elementType = elementType;
  return type;
}
...
{
  if (type1->typeClass == TP_STRING && type2->typeClass == TP_CHAR)
  {
    return 1;
  }
  else if (type1->typeClass == TP_CHAR && type2->typeClass == TP_STRING)
  {
    return 1;
  }

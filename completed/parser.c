...
Type *compileType(void)
{
  ...
  //   arraySize = currentToken->value;
  //   type = makeStringType(arraySize, elementType);
  //   break;
  case TK_IDENT:
    eat(TK_IDENT);
...
Type *compileBasicType(void)
{
  ...
  // case KW_STRING:
  //   eat(KW_STRING);
  //   type = makeStringType(0, makeCharType());
  //   break;
  default:
...
void compileCondition(void)
{
  ...
  printf("Check: %d - %d", type1->typeClass, type2->typeClass);
...
Type *compileExpression(void)
{
  ...
  case SB_MINUS:
    ...
    type = compileExpression2();
    checkIntType(type);
    printf("Check type: %d", type->typeClass);
    genNEG();
    break;
  default:
    type = compileExpression2();
